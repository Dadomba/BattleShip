...
public class AI {
	...
	public static Grid autoGeneratedGrid(int ship_amount[]) throws Exception {
		...
		if (ship_amount.length != maxAmount.length) {
			throw new Exception(
					"Lenght of ship_amount isn\'t equal to "+maxAmount.length+". Must be in this order: Aircraft Carrier, Battleship, Cruiser, Destroyer, Submarine.");
		}

		for (int i = 0; i < maxAmount.length; i++) {
			if (ship_amount[i] > maxAmount[i]) {
				String name = typeList.get(i).getClass().getSimpleName();
				throw new Exception("The ship amount of " + name
						+ " specified in parameter is greater than "
						+ maxAmount[i] + " defined in the file Constant.java");
			}
		}
			...
			for (int j = 0; j < ship_amount[i]; j++) {
				...
				do {
					...
					try {
						...
						System.err
								.println("Failed to place the random generated "
										+ cl.getSimpleName()
										+ " "
										+ (j + 1)
										+ ". Let's try again !");
