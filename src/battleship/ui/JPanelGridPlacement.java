...
public class JPanelGridPlacement extends JPanel implements MouseListener,
		MouseMotionListener {
	...
	public void tryPlaceShip(int idShip) {
		try {
			...
			int orientation;
			if (idShip != 4)
				orientation = JOptionPane.showOptionDialog(this,
						"Orientation ? ", "Orientation",
						JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE, null, orientations, 0);
			else
				orientation = 0;
			...
			if (idShip == 0)
				...
				currentShipToPlace = new Battleship(new Coord(0, 0),
						orientation);
			else if (idShip == 2)
	...
	public void mouseClicked(MouseEvent arg0) {
		if (currentShipToPlace != null && isCorrectlyPositioned()) {
			try {
				shipPlaced[Math.abs(currentShipToPlace.getSize()
