...
public class CreateGridFrame extends JFrame implements WindowListener {
	...
	private void updateJButtonText() {
		...
		int nbBattleshipLeft = Constant.BATTLESHIPMAX
				- jp_grid.getNbShipPlaced(1);
		int nbCruiserLeft = Constant.CRUISERMAX - jp_grid.getNbShipPlaced(2);
		int nbDestroyerLeft = Constant.DESTROYERMAX
				- jp_grid.getNbShipPlaced(3);
		int nbSubmarineLeft = Constant.SUBMARINEMAX
				- jp_grid.getNbShipPlaced(4);

		jb_addAircraft.setText("Add aircraft (size = " + Constant.AIRCRAFTSIZE
				+ "). " + nbAircraftLeft + " left");
		jb_addBattleship.setText("Add battleship (size = "
				+ Constant.BATTLESHIPSIZE + "). " + nbBattleshipLeft + " left");
		jb_addCruiser.setText("Add cruiser (size = " + Constant.CRUISERSIZE
				+ "). " + nbCruiserLeft + " left");
		jb_addDestroyer.setText("Add destroyer (size = "
				+ Constant.DESTROYERSIZE + "). " + nbDestroyerLeft + " left");
		jb_addSubmarine.setText("Add submarine (size = "
				+ Constant.SUBMARINESIZE + "). " + nbSubmarineLeft + " left");
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					Game.getInstance().setPlayer(
							new Player(playerName, AI
									.autoGeneratedGrid(Constant.SHIP_AMOUNT)));
					dispose();
				} catch (Exception e) {
					e.printStackTrace();
				}
